...
do
    ...
    cat ${query} | seqkit grep -r -p ^${sample} -v  | blastp -db ${database} -out ${blastoutput} -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovhsp qlen slen' -task 'blastp' -num_threads ${threads} -max_target_seqs '1' -max_hsps '1' -evalue ${evalue}
